...
import static jlab.tvchannels.DetailChannelActivity.TYPE_TV;
...
public class TVFragment extends Fragment implements APIController.ResponseServerCallback {
    ...
    private final BroadcastReceiver loadChannelsReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if(action != null && action.equals(Utils.LOAD_TV_CHANNELS_ACTION))
                loadChannels();
        }
    };
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        LocalBroadcastManager.getInstance(getContext())
                .registerReceiver(loadChannelsReceiver, new IntentFilter(Utils.LOAD_TV_CHANNELS_ACTION));
    }
    ...
    public void onChannelsLoadServer(ArrayList<Country> countries) {
        ...
        intent.putExtra(Utils.IS_TV_KEY, true);
        ...
        for (int i = 0; i < countries.size(); ++i) {
            ...
            for (int j = 0; j < communities.size(); ++j) {
                ...
                if (isShowingFavorites) {
                    ...
                    boolean isCommunityShown = SourcesManagement.isTVCommunitySelected
                            ("" + communities.get(j).getName());
                    if (isCommunityShown) {
                        for (int k = 0; k < channels.size(); ++k) {
                            channelLists.add(new ChannelList(countries.get(i).getName(),
                                    communities.get(j).getName(), channels.get(k)));
                                            ...
                                            if (linearLayout.getChildAt(j) instanceof LinearLayout) {
                                                ...
                                                for (int z = 0; z < columnLayout.getChildCount(); ++z) {
                                                    if (columnLayout.getChildAt(z) instanceof CheckBox) {
                                                        ...
                                                        SourcesManagement.setTVCommunitySelected
                                                                ("" + checkBox.getText(), checkBox.isChecked());
                                                        createChannelList();
                                                    }
                                                }
                                            }
